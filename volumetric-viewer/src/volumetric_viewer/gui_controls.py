...
# List to hold points for the transfer function
points = []
...
def update_transfer_plot(from_outside = False, event_queue=None):
    ...
    #TODO: Notify changes in plot to renderer
    # if not from_outside:
    #     event_queue.push(TransferFunctionUpdateEvent())
...
# Function for middle-click interaction (select a point)
def on_middle_click(x, y):
    ...
    for i, (px, py) in enumerate(points):
        if abs(px - x) < tolerance_x and abs(py - y) < tolerance_y:
            dpg.set_value("selected_point", i)  # Select the point
            print(f"Point selected: {i} ({px}, {py})")
            return
...
# Mouse drag callback to move the selected point
def mouse_drag_callback(sender, app_data):
    selected = dpg.get_value("selected_point")
    if selected == -1 or selected >= len(points):
        return 
...
def transfer_function_editor():
    with dpg.group(tag="transfer_function_editor_group"):
        with dpg.plot(tag="tf_plot", height=400, width=500, no_menus=True, no_box_select=True):
            ...
            with dpg.plot_axis(dpg.mvYAxis, tag="y_axis", label="Opacity", lock_min=True, lock_max=True):
                dpg.set_axis_limits("y_axis", -0.5, 1.5)
            ...
            with dpg.handler_registry():
                ...
                dpg.add_mouse_drag_handler(button=dpg.mvMouseButton_Middle, callback=mouse_drag_callback) 
                dpg.add_mouse_release_handler(callback=mouse_release_callback)
    with dpg.group(tag="editor_btns"):
        dpg.add_button(label="Clear Points", tag="clear_points_btn", callback=clear_points) 
